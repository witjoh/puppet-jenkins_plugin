...
class Actions {
  ...
  # Matrix sexurity can be set using the jenkins_authorization_strategy
  # We need to add the jenkins admin user in the same time we enable the 
  # matrix, otherwise we cannot configure jenkins using the cli/puppet
...
*/
  ...
  void matrix_get() {
    util.requirePlugin('matrix-auth')
    def j = Jenkins.get()
    util.printAllMethods(j)
  }   
